...
function createHTMLDocument(content: string, title: string = 'Page') {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body class="bg-background text-foreground">
  ${content}
  <script defer src="${SITE_URL}/darkmode.js"></script>
</body>
</html>`
}
...
async function generateDynamicPages(route: RouteInfo, outputDir: string) {
  ...
  const baseDir = join(outputDir, routeBase)
  ...
  for (const item of dataForRoute) {
    try {
      ...
      const itemDir = join(baseDir, item.slug)
      if (!existsSync(itemDir)) {
        mkdirSync(itemDir, { recursive: true })
      }
      
      // Save the page
      const itemPath = join(itemDir, 'index.html')
